...
def run_column_validation(client, src_ref, tgt_ref, common_cols, log_print):
    ...
    for col in common_cols:
        ...
        agg_exprs.append(f"COUNTIF(`{col}` IS NULL) AS {safe_col}__null_cnt")
        ...
        if is_numeric:
            agg_exprs.append(f"SUM(`{col}`) AS {safe_col}__sum")
            agg_exprs.append(f"AVG(`{col}`) AS {safe_col}__avg")
            agg_exprs.append(f"MIN(`{col}`) AS {safe_col}__min")
            agg_exprs.append(f"MAX(`{col}`) AS {safe_col}__max")
    ...
    try:
        ...
        log_print(f"   ❌ Failed to execute aggregates for column validation: {e}")
        return []
    ...
    for col, metric, alias in agg_mappings:
        ...
        log_print(f"      {status_icon} Column: '{col}' | Metric: {metric:10} | Source: {src_val} | Target: {tgt_val}")
...
def run_row_level_validation(client, src_ref, tgt_ref, audit_ref, common_cols, run_timestamp):
    ...
    pk_partition_src = ", ".join([f"`{pk}`" for pk in PRIMARY_KEYS])
    pk_partition_tgt = ", ".join([f"`{pk}`" for pk in PRIMARY_KEYS])
    ...
    WITH src_data AS (
        ...
        -- Deduplicate source PKs prior to JOIN to avoid Cartesian duplication
        QUALIFY ROW_NUMBER() OVER (PARTITION BY {pk_partition_src}) = 1
    ),
    tgt_data AS (
        SELECT 
        ...
        -- Deduplicate target PKs prior to JOIN to avoid Cartesian duplication
        QUALIFY ROW_NUMBER() OVER (PARTITION BY {pk_partition_tgt}) = 1
    )
    SELECT
...
with DAG(
    dag_id="bq_data_validation_tool",
