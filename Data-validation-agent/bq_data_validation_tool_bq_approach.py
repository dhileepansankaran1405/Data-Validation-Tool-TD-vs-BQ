...
# =====================================================================
# LOCAL FILE LOGGING UTILITY
# Intercepts console print statements and duplicates them to DVT_Results.txt
# =====================================================================
OUTPUT_FILE = "DVT_Results.txt"

# Clear any previous results from the file at the start of a run
try:
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write("")
except Exception as e:
    sys.stderr.write(f"⚠️ Warning: Could not initialize local log file {OUTPUT_FILE}: {e}\n")

# Override standard print function globally
def dual_print(*args, **kwargs):
    """Prints to the terminal console AND writes to the local text file."""
    ...
    try:
        ...
        sys.stderr.write(f"Error writing to local file {OUTPUT_FILE}: {e}\n")

# Assign global override
print = dual_print
...
def run_row_level_validation(client, src_ref, tgt_ref, audit_ref, common_cols, run_timestamp):
    ...
    # Safely pass the parameterized timestamp
...
def fetch_and_print_mismatch_details(client, audit_ref, run_timestamp, limit=10):
    """Queries only the data matching the current run's timestamp to prevent historical duplicates."""
    print("\n🔍 Fetching mismatch records from this validation run...")
    query = f"""
    SELECT 
      validation_time,
      primary_key_value,
    ...
    try:
        ...
        print("\n=========================================================")
        print(f"📁 DETAILED ROW-LEVEL MISMATCHES FOR THIS RUN ({len(rows)} Records)")
        print("=========================================================")
        for idx, row in enumerate(rows, start=1):
            print(f"\n👉 RECORD #{idx}")
            print(f"  🔑 Primary Key Value : {row.primary_key_value}")
            print(f"  ❌ Mismatch Type     : {row.mismatch_type}")
            print(f"  🌐 Source Hash       : {row.source_row_hash}")
            print(f"  🌐 Target Hash       : {row.target_row_hash}")
            print(f"  📦 Record Payloads (JSON Structs):")
            print(f"     {row.details}")
        print("=========================================================\n")
    except Exception as e:
        print(f"⚠️ Could not pull detailed row metadata: {e}")
...
def main():
    ...
    # Establish a unique timezone-aware UTC timestamp for the entire duration of this run
    ...
    print("=========================================================")
    print(f"  📅 Execution Time     : {run_timestamp.strftime('%Y-%m-%d %H:%M:%S')} (UTC)")
    ...
    if total_discrepancies_inserted > 0:
        fetch_and_print_mismatch_details(client, audit_ref, run_timestamp, limit=10)

    print(f"💾 End-to-End run logged successfully! You can find the log output file locally at:")
    print(f"   📂 {OUTPUT_FILE}\n")
